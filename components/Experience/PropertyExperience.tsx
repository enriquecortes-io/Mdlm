          ...
          <div style={{
            ...
            <div style={{ display:"flex", flexDirection:"column" }}>
              ...
              <div style={{
                ...
                {/* Precio vertical */}
                {property.precio && (
                  <div style={{ flex:"1 1 100%", padding:"0.5rem 1rem 0.5rem 0", borderTop:"1px solid rgba(255,255,255,0.06)", marginTop:"1rem", paddingTop:"1.2rem" }}>
                    <p style={{ fontFamily:"'Montserrat',sans-serif", fontSize:"0.72rem", fontWeight:700, color:"#8a7550", letterSpacing:"0.25em", textTransform:"uppercase", margin:"0 0 1.8rem" }}>Precio</p>
                    <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(1.3rem,2.4vw,1.9rem)", color:"#a8842f", fontWeight:500, margin:0, lineHeight:1.1 }}>€{property.precio/1000000 % 1 === 0 ? (property.precio/1000000).toFixed(0) : (property.precio/1000000).toFixed(3).replace(/\.?0+$/, "")}M</p>
                  </div>
                )}
              </div>
            ...
            {images.length > 1 && (
              <>
                <button
                  ...
                  style={{ position:"absolute", left:"-3rem", top:"50%", transform:"translateY(-50%)", background:"none", border:"1px solid rgba(26,23,20,0.2)", color:"rgba(26,23,20,0.5)", width:"2.2rem", height:"2.2rem", cursor:"pointer", fontSize:"0.9rem", display:"flex", alignItems:"center", justifyContent:"center" }}
                >←</button>
                <button
                  onClick={e => { e.stopPropagation(); const i = images.indexOf(lightbox); setLightbox(images[(i+1)%images.length]); }}
                  style={{ position:"absolute", right:"-3rem", top:"50%", transform:"translateY(-50%)", background:"none", border:"1px solid rgba(26,23,20,0.2)", color:"rgba(26,23,20,0.5)", width:"2.2rem", height:"2.2rem", cursor:"pointer", fontSize:"0.9rem", display:"flex", alignItems:"center", justifyContent:"center" }}
                >→</button>
              </>
            )}
